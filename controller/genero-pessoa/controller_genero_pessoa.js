 ...
 *          para o CRUD de gêneros de pessoas.
 * Data: 27/10/2025
 * Autor: Nathan
 * Versão: 1.0
 ******************************************************************************/

// Import da model do DAO do genero-pessoa
const generoDAO = require('../../model/DAO/genero-pessoa.js')
...
const listarGeneros = async () => {
    ...
    try {
        ...
        let resultGeneros = await generoDAO.getSelectAllPersonGender()

        if (resultGeneros) {
            if (resultGeneros.length > 0) {
                MESSAGES.DEFAULT_HEADER.status = MESSAGES.SUCCESS_REQUEST.status
                MESSAGES.DEFAULT_HEADER.status_code = MESSAGES.SUCCESS_REQUEST.status_code
                MESSAGES.DEFAULT_HEADER.items.cargos = resultGeneros
...
const buscarGeneroId = async (id) => {
    ...
    try {
        ...
        if (!isNaN(id) && id != '' && id != null && id > 0) {
            let resultGeneros = await generoDAO.getSelectbyIdPersonGender(Number(id))

            if (resultGeneros) {
                if (resultGeneros.length > 0) {
                    MESSAGES.DEFAULT_HEADER.status = MESSAGES.SUCCESS_REQUEST.status
                    MESSAGES.DEFAULT_HEADER.status_code = MESSAGES.SUCCESS_REQUEST.status_code
                    MESSAGES.DEFAULT_HEADER.items.cargo = resultGeneros
    ...
    try {
        ...
        if (String(contentType).toUpperCase() == 'APPLICATION/JSON') {
            ...
            if (!validar) {
                ...
                let resultGeneros = await generoDAO.setInsertPersonGender(genero)
        ...
        if (String(contentType).toUpperCase() == 'APPLICATION/JSON') {
            ...
            if (!validar) {
                ...
                if (validarID.status_code == 200) {
                    ...
                    // Chama a função para inserir um novo genero no BD
                    let resultGenero = await generoDAO.setUpdatePersonGender(genero)
...
const excluirGenero = async (id) => {
    ...
    try {
        ...
        if (validarID.status_code == 200) {
            ...
            // Chama a função para inserir um novo genero no BD
            let resultGeneros = await generoDAO.setDeletePersonGender(id)
